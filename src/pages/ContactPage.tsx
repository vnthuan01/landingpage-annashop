  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--color-bg)", paddingTop: "9rem", paddingBottom: "7rem" }}>
      <div className="container-main">
        ...
        >
          <p style={{ fontSize: "0.75rem", letterSpacing: "0.4em", color: "var(--color-accent-blue)", marginBottom: "1rem", textTransform: "uppercase", fontWeight: 300 }}>
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                <div style={{ display: "flex", gap: "1rem" }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--color-accent-blue)" strokeWidth="1.5" style={{ flexShrink: 0, marginTop: "0.25rem" }}>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.5rem", flex: 1 }}>
                <div>
                  ...
                  <input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={inputStyle} placeholder="Nhập họ và tên của bạn" />
                </div>
                <div>
                  <label style={labelStyle}>Email *</label>
                  <input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} style={inputStyle} placeholder="Nhập địa chỉ email" />
                </div>
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>Nội dung *</label>
                  <textarea required value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} rows={5} style={{ ...inputStyle, resize: "none", height: "calc(100% - 2rem)" }} placeholder="Bạn cần hỗ trợ gì?" />
                </div>
